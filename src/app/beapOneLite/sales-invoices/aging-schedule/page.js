...
const mockOutstandingInvoices = [
  ...
  const today = new Date("12/05/2025"); // mock "today" for consistency
 ...
 const filteredInvoices = mockOutstandingInvoices
  .filter((inv) =>
    inv.customer.toLowerCase().includes(searchQuery.toLowerCase()) ||
    inv.id.toLowerCase().includes(searchQuery.toLowerCase())
  )
  .map((inv) => {
    const aging = getAgingDetails(inv.due);
    return { ...inv, ...aging };
  });

  // Apply sorting
filteredInvoices.sort((a, b) => {
  if (selectedSort === "Days Past Due") {
    return b.diffDays - a.diffDays;
  }
  if (selectedSort === "Amount (High to Low)") {
    return b.amountUSD - a.amountUSD;
  }
  if (selectedSort === "Customer Name") {
    ...
    <div className="p-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-8">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border flex justify-between items-center">
          <div>
            ...
            <p className="text-2xl font-semibold">{stats.totalOutstanding}</p>
            <p className="text-xs text-gray-500 mt-1">1 invoices</p>
          </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border flex justify-between items-center">
          <div>
            ...
            <p className="text-2xl font-semibold">{stats.avgOverdue}</p>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border flex justify-between items-center">
          <div>
            ...
            <p className="text-2xl font-semibold">{stats.current}</p>
      ...
      <div className="bg-white p-6 rounded-xl border shadow-sm mt-10">
        <h2 className="text-lg font-semibold mb-6 flex items-center gap-2">
          <span><BarChart3 size={26}/></span> Aging Breakdown
        </h2>
  ...
  <div className="mt-6">
    ...
    {filteredInvoices.map((inv) => (
      ...
      >
        ...
        <button className="border rounded-lg p-2 ml-4 hover:bg-gray-100">
          <svg
            className="w-5 h-5"
            fill="none"
            stroke="currentColor"
